{{-- =========================================================
TESTIMONIAL SECTION - SINGLE CARD CAROUSEL
========================================================= --}}
        ...
        <div
            ...
            <div class="relative mx-auto max-w-4xl">
                ...
                @foreach($testimonials as $index => $testimonial)
                    <div x-show="active === {{ $index }}" x-cloak x-transition:enter="transition ease-out duration-500" x-transition:enter-start="opacity-0 translate-x-8" x-transition:enter-end="opacity-100 translate-x-0" x-transition:leave="transition ease-in duration-300" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0 -translate-x-8" class="w-full">
                            ...
                            <div class="flex items-center gap-5 pr-16 sm:pr-20">
                                @if(filled($testimonial->photo))
                                    <div class="relative shrink-0">
                                        ...
                                        <img src="{{ Storage::url($testimonial->photo) }}" alt="{{ $testimonial->name }}" loading="lazy" class="relative h-16 w-16 rounded-full border-4 border-white object-cover shadow-md sm:h-20 sm:w-20">
                                    </div>
                                @else
            ...
            @if($testimonials->count() > 1)
                <div class="mt-8 flex items-center justify-center gap-2">
                    @foreach($testimonials as $index => $testimonial)
                        <button type="button" @click="goTo({{ $index }})" aria-label="Buka testimoni {{ $index + 1 }}" :class="active === {{ $index }} ? 'w-10 bg-emerald-600' : 'w-2.5 bg-slate-300 hover:bg-emerald-300'" class="h-2.5 rounded-full transition-all duration-300"></button>
                    @endforeach
                </div>
            @endif
        </div>
